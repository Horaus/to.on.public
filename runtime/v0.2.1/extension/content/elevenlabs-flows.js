(function() {
	//#region ../../packages/extension-providers/src/elevenlabs-flows/content.ts
	console.log("[Studio] ElevenLabs Flows adapter loaded");
	var ADAPTER_VERSION = "2026-07-20-elevenlabs-flows-v1";
	var NODE_SELECTOR = ".react-flow__node[data-id]";
	...
	function nodeKind(node) {
		...
		if (className.includes("react-flow__node-content-generation")) {
			...
			if (Array.from(node.querySelectorAll(".react-flow__handle[data-handleid]")).map((handle) => handle.getAttribute("data-handleid")).includes("referenceVideos") || /^\s*Video/i.test(text)) return "video-generation";
			if (/^\s*Image/i.test(text)) return "image-generation";
			return "generation";
		}
		return className.match(/react-flow__node-([\w-]+)/)?.[1] || "unknown";
	}
	...
	function mediaUrls(node) {
		...
		return Array.from(node.querySelectorAll(selector)).filter((element) => mediaElementMatchesKind(element, kind)).map(mediaElementUrl).filter(isUsableMediaUrl).filter((url) => kind !== "image-generation" || isGeneratedImageUrl(url));
	}
	function mediaElementMatchesKind(element, kind) {
		return kind !== "image-generation" || element instanceof HTMLImageElement && element.className.includes("object-contain") && !element.className.includes("blur");
	}
	...
	function isGeneratedImageUrl(url) {
		return url.includes("/content_generation/") && !url.includes("/content_generation/content_asset/");
	}
	function mediaReady(node, candidate) {
		const canonicalCandidate = canonicalMediaUrl(candidate);
		return Array.from(node.querySelectorAll("img, video")).some((element) => {
			if (canonicalMediaUrl(element instanceof HTMLVideoElement ? element.currentSrc || element.src : element instanceof HTMLImageElement ? element.currentSrc || element.src : "") !== canonicalCandidate) return false;
			if (element instanceof HTMLImageElement) return element.complete && element.naturalWidth >= 256 && !element.className.includes("blur");
			return element instanceof HTMLVideoElement && element.readyState >= HTMLMediaElement.HAVE_METADATA;
		});
	}
	function generationBlocker() {
		const dialog = Array.from(document.querySelectorAll("[role=\"dialog\"], [data-radix-dialog-content]")).find((candidate) => visible(candidate) && /upgrade to continue creating|subscribe to creator|insufficient credits|not enough credits/i.test(candidate.innerText || ""));
		if (!dialog) return "";
		return (dialog.innerText || "").replace(/\s+/g, " ").trim().slice(0, 360);
	}
	function inspectFlow() {
		return Array.from(document.querySelectorAll(NODE_SELECTOR)).map((node) => ({
			...
			handles: Array.from(node.querySelectorAll(".react-flow__handle[data-handleid]")).map((handle) => handle.getAttribute("data-handleid") || "").filter(Boolean),
			mediaUrls: mediaUrls(node)
		}));
	}
	function generationKind(payload) {
	...
	function exactNode(nodeId) {
		if (!nodeId) return null;
		return Array.from(document.querySelectorAll(NODE_SELECTOR)).find((node) => node.getAttribute("data-id") === String(nodeId)) || null;
	}
	function resolveGenerationNode(payload) {
		const expectedKind = generationKind(payload);
		const requested = exactNode(payload.settings?.generationNodeId);
		if (requested) {
			if (nodeKind(requested) !== expectedKind) throw new Error(`Node ${requested.dataset.id} is ${nodeKind(requested)}, expected ${expectedKind}.`);
			return requested;
		}
		const candidates = Array.from(document.querySelectorAll(NODE_SELECTOR)).filter((node) => nodeKind(node) === expectedKind);
		if (candidates.length !== 1) throw new Error(`Expected one ${expectedKind} node, found ${candidates.length}. Set settings.generationNodeId.`);
		return candidates[0];
	}
	function incomingHandleIsConnected(nodeId, handleId) {
		return Array.from(document.querySelectorAll(".react-flow__edge[data-testid]")).some((edge) => (edge.getAttribute("data-testid") || "").endsWith(`-${nodeId}-${handleId}`));
	}
	function assertGenerationContract(node, payload) {
		const id = node.dataset.id || "";
		if (!incomingHandleIsConnected(id, "prompt")) throw new Error(`Generation node ${id} has no prompt edge. Connect a Text or LLM output first.`);
		if (payload.settings?.requireReference !== false && !incomingHandleIsConnected(id, "referenceImages") && !incomingHandleIsConnected(id, "referenceVideos")) throw new Error(`Generation node ${id} has no reference media edge.`);
	}
	function resolvePromptNode(payload, generationNode) {
		const requested = exactNode(payload.settings?.promptNodeId);
		if (requested) return requested;
		const generationId = generationNode.dataset.id || "";
		const testId = Array.from(document.querySelectorAll(".react-flow__edge[data-testid]")).find((candidate) => (candidate.getAttribute("data-testid") || "").endsWith(`-${generationId}-prompt`))?.getAttribute("data-testid") || "";
		const node = exactNode(Array.from(document.querySelectorAll(`${NODE_SELECTOR}.react-flow__node-text`)).map((node) => node.dataset.id || "").find((id) => testId.includes(`edge-${id}-${generationId}-prompt`)));
		if (!node) throw new Error(`Could not resolve the Text node connected to ${generationId}. Set settings.promptNodeId.`);
		return node;
	}
	async function replacePrompt(promptNode, prompt) {
		const editor = promptNode.querySelector("[contenteditable=\"true\"]");
		...
		await sleep(1200);
		if ((editor.textContent || "").trim() !== prompt.trim()) throw new Error("ElevenLabs prompt editor reverted the requested text after its state update.");
	}
	function runButton(node) {
		const button = Array.from(node.querySelectorAll("button")).find((candidate) => visible(candidate) && /^run$/i.test((candidate.textContent || "").trim()));
	...
	async function panCanvasTowardNode(node, rect, pane, paneRect, margin) {
		if (!pane || !paneRect) throw new Error(`Could not reveal ElevenLabs node ${node.dataset.id}: canvas pane is missing.`);
		const moveMode = document.querySelector("button[aria-label=\"Move\"]");
		...
		const maxDrag = 320;
		const deltaX = Math.max(-320, Math.min(maxDrag, desiredX));
		const deltaY = Math.max(-320, Math.min(maxDrag, desiredY));
	...
	async function waitForResult(jobId, node, before, maxWaitMs) {
		...
		while (Date.now() - startedAt < maxWaitMs) {
			...
			if (blocker) {
				reportResult(jobId, "failed_manual", void 0, `ElevenLabs blocked generation: ${blocker}`);
				return;
			}
			...
			const progress = Math.min(.92, .35 + (Date.now() - startedAt) / maxWaitMs * .57);
		...
		reportResult(jobId, "failed_retryable", void 0, `Timed out waiting for output from ElevenLabs node ${node.dataset.id}.`);
	}
	function isStableResultReady(args) {
		return !args.loading && args.ready && Boolean(args.candidate) && args.stablePolls >= 2 && (args.created.length > 0 || args.observedRunning || args.before.size === 0);
	}
	function updateResultStability(loading, ready, candidate, stableCandidate, stablePolls) {
	...
	async function createNode(kind) {
		...
		const button = Array.from(document.querySelectorAll(`button[aria-label="${label}"]`)).find(visible);
	...
	async function executeJob(payload) {
		try {
			...
			reportStatus(payload.jobId, "submitting", `Updating prompt node ${promptNode.dataset.id}...`, .25);
			...
			reportStatus(payload.jobId, "generating", `Started ${generationNode.dataset.id}.`, .35);
			await waitForResult(payload.jobId, generationNode, before, Number(payload.settings?.maxWaitMs || 6e5));
		} catch (error) {
			reportResult(payload.jobId, "failed_retryable", void 0, `ElevenLabs Flows automation error: ${error instanceof Error ? error.message : String(error)}`);
		}
	}
	...
	chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
		...
		if (message.action === "CREATE_ELEVENLABS_FLOW_NODE") {
			createNode(message.kind === "image-generation" ? "image-generation" : "video-generation").then((node) => sendResponse({
				ok: true,
				adapter: "elevenlabs-flows",
				node
...
//# sourceMappingURL=elevenlabs-flows.js.map