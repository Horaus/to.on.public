	...
	*/
	...
	async function getOrUploadReferenceTile(jobId, references, requireDirectFrameAttachment = false, skipReuse = false) {
		...
		try {
			...
			const existing = await findExistingUploadedReferenceTileByVisual(jobId, context.reference);
			if (existing && await waitForTileReady(existing, 5e3)) {
				flowTrace(jobId, `Flow upload boundary unavailable; reusing the exact existing media tile (${compactText(tileSearchText(existing), 90)}).`, .36);
				rememberFlowTileForReference(context.reference, existing);
				return {
					attachedDirectly: false,
					tile: existing,
